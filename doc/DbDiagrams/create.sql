...
CREATE TABLE IF NOT EXISTS `HilfeDb`.`Notifica` (
  ...
  `idUserGeneratore` INT NOT NULL,
  PRIMARY KEY (`idNotifica`, `idUser`),
  INDEX `fk_Notifica_User1_idx` (`idUser` ASC) ,
  INDEX `fk_Notifica_User2_idx` (`idUserGeneratore` ASC) ,
  CONSTRAINT `fk_Notifica_User1`
    FOREIGN KEY (`idUser`)
    REFERENCES `HilfeDb`.`User` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Notifica_User2`
    FOREIGN KEY (`idUserGeneratore`)
    REFERENCES `HilfeDb`.`User` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
